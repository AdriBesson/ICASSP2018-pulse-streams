function [x,z,bmode,migSIG,param] = fkmig(SIG,param)
...
%   
%   -- Damien Garcia & Louis Le Tarnec -- 2011/08, revised 2013/10/04
%   website: <a
%   href="matlab:web('http://www.biomecardio.com')">www.BiomeCardio.com</a>

%--- Display a Warning dialog once in the current session
% persistent IsBeingUsedFKMIG
% if isempty(IsBeingUsedFKMIG)
%     h = warndlg({'Please refer to the following paper when using this code:','',...
%         'Garcia et al., Stolt''s f-k migration for plane wave ultrasound imaging.',...
%         'IEEE Trans Ultrason Ferroelectr Freq Control, 2013;60:1853-1867.',...
%         '','Contact: Garcia.Damien@gmail.com'},...
%         'IMPORTANT NOTE');
%     uiwait(h)
%     IsBeingUsedFKMIG = true;
%     close(gcf)
% end
...
nxFFT = round(1.5*nx); % in order to avoid lateral edge effects
...
for k = 1:Nframes
    ...
    fkz = v*sqrt(kx.^2+4*f.^2/c^2*C^2);   %-- Garcia method
%     fkz = f + (c*kx).^2 ./ (4*f);           %-- Lu method
%     fkz(f==0)=0;
...
bmode = 20*log10(env/max(max(env)));
% bmode = 255/dBRange*(bmode + dBRange);
% bmode = uint8(255*bmode/max(max(bmode)));
